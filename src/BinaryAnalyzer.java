...
public class BinaryAnalyzer {
    ...
    public ArrayList<Integer> publicFunctionStartList = new ArrayList<>(); // all the functions that can be called by
                                                                           // others. Fallback function is not included
    ...
    public BinaryAnalyzer(String bytecode) throws Exception {
        if (init(bytecode)) {
            getBasicBlock();
            addFallEdges();
            buildCFG();
    ...
    private boolean init(String bytecode) throws Exception {
    ...
    private void getBasicBlock() {
        ...
        for (int i = 0; i < this.disasm.size(); i++) {
            ...
            if (start || instr.trim().equals("JUMPDEST")) {
                if (i != 0) {
            ...
            else if (instr.trim().equals("JUMPI")) {
                start = true;
                block.jumpType = BasicBlock.CONDITIONAL;

            }

            else if (instr.trim().equals("JUMP")) {
                start = true;
                block.jumpType = BasicBlock.UNCONDITIONAL;
            }

            // some blocks may only have one instr, so we use if rathar than else if
            if (instr.trim().equals("STOP") || instr.trim().equals("RETURN") || instr.trim().equals("REVERT")
                    || instr.trim().equals("SELFDESTRUCT")
                    || instr.trim().equals("ASSERTFAIL")) {
                start = true;
