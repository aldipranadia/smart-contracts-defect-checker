...
public class DefectChecker {
    ...
    public boolean detectGreedyContract() {
        // if a contract can receive money(contains payable function), but cannot send
        // ETH(do not contains CALL/SELFDESTRUCT)
    ...
    public boolean detectReentrancy() {
        ...
        // Step4: detect whether the address is modified before the "CALL"
        // instr.(SSTORE_PC && CALL_PC)

        // findCallPath(new ArrayList<>(), this.binaryAnalyzer.pos2BlockMap.get(0), new
        // HashSet<>());
        for (ArrayList<Integer> path : this.binaryAnalyzer.allCallPath) {
            ...
            // this path does not read from storage ==> this path does not have reentrancy
            // bug
            if (sloadPC == -1)
                continue;
            ...
            for (int i = 0; i < this.binaryAnalyzer.stackEvents.size() - 1; i++) {
                ...
                if (eventLog[0].equals("SSTORE")) {
                    ...
                    if (Utils.legalRange(pc, legalRange)) {
                        ...
                        if (top.length() > 0) {
                            ...
                            // SSTORE read the same address with SLOAD, if sstorePC < CallPC ==> do not have
                            // reentrancy
                ...
                if (eventLog[0].equals("CALL")) {
                    ...
                    if (Utils.legalRange(pc, legalRange)) {
                        ...
                        String top = tmp[2].split(" ")[0];
                        if (top.length() > 0) {
                            // detect gas limitation, if CALL is created by send or transfer, then it will
                            // limit gas to 2300
